...
def get_batch(filename,batch_size,input_dim,\
    ...
    def parser_train(record):
        ...
        if(transform_fn is None):
            ...
            inp,outp=transform_fn(sequence['inputs'],sequence['outputs'],sequence['noises'])
        splice_inputs = splice_feats(inp, left_context, right_context)
        return context['name'], splice_inputs, outp
    ...
    def parser_infer(record):
        ...
        if(transform_fn is None):
            ...
            inp,outp=transform_fn(sequence['inputs'],None,None)
        splice_inputs = splice_feats(inp, left_context, right_context)
    ...
    if(not infer):
        ...
        dataset=dataset.map(lambda name,inputs,outputs:\
                (name,inputs,outputs,tf.shape(inputs)[0]),num_parallel_calls=num_threads)
        ...
        name,inputs,outputs,lengths=dataset_iter.get_next()
        return name,inputs,outputs,lengths
    else:
